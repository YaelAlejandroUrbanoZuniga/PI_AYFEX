...
import {
  ...
  ScrollView, StatusBar, Modal, TextInput, Platform,
} from 'react-native';
...
export default function PerfilM({ navigation }) {
  ...
  const guardarPerfil = async () => {
    if (!nombreEditar || nombreEditar.length < 3) return;
    if (!correoEditar || !correoEditar.includes('@')) return;
    if (!telefonoEditar || telefonoEditar.length < 7) return;
  ...
  const enviarReporte = async () => {
    if (!reporteTipo || reporteTipo.length < 3) {
      setAlertTitulo("Campo requerido");
      setAlertMensaje("El tipo de reporte debe tener al menos 3 caracteres.");
      setAlertVisible(true);
      return;
    }
    if (!reporteDescripcion || reporteDescripcion.length < 10) {
      setAlertTitulo("Campo requerido");
      setAlertMensaje("La descripción debe tener al menos 10 caracteres.");
      setAlertVisible(true);
      return;
    }
    ...
    try {
      ...
      setAlertTitulo("Error al enviar");
      setAlertMensaje(error.message || "No se pudo conectar con el servidor.");
      setAlertVisible(true);
    } finally {
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionHeader}>
          ...
          <TouchableOpacity style={styles.editButton} onPress={() => setModalEditarVisible(true)}>
        ...
        <TouchableOpacity style={styles.actionItem} onPress={() => setModalReporteVisible(true)}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <View style={styles.inputContainer}>
              <Ionicons name="person-outline" size={18} color="#FF6B00" />
              <TextInput style={styles.input} value={nombreEditar} onChangeText={setNombreEditar} placeholder="Tu nombre completo" placeholderTextColor="#BBB" />
            </View>

            <Text style={styles.inputLabel}>Correo electrónico</Text>
            <View style={styles.inputContainer}>
              <Ionicons name="mail-outline" size={18} color="#FF6B00" />
              <TextInput style={styles.input} value={correoEditar} onChangeText={setCorreoEditar} placeholder="Tu correo" placeholderTextColor="#BBB" keyboardType="email-address" autoCapitalize="none" />
            </View>

            <Text style={styles.inputLabel}>Teléfono</Text>
            <View style={styles.inputContainer}>
              <Ionicons name="call-outline" size={18} color="#FF6B00" />
              <TextInput style={styles.input} value={telefonoEditar} onChangeText={setTelefonoEditar} placeholder="Tu teléfono" placeholderTextColor="#BBB" keyboardType="phone-pad" />
            </View>
            ...
            >
              <Text style={styles.saveButtonText}>{cargandoEditar ? "Guardando..." : "Guardar cambios"}</Text>
            </TouchableOpacity>
      ...
      <Modal visible={modalReporteVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContainer, styles.modalReporteContainer]}>

            {reporteExitoso ? (
              <View style={styles.exitoContainer}>
                <View style={styles.exitoIconContainer}>
                  <Ionicons name="checkmark-circle" size={52} color="#34C759" />
                </View>
                <Text style={styles.exitoTitle}>Reporte enviado</Text>
                <Text style={styles.exitoSubtitle}>Tu reporte fue registrado correctamente</Text>
              </View>
            ) : (
              <>
                <View style={styles.modalHeader}>
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                  ...
                  <View style={[styles.inputContainer, { alignItems: 'flex-start', minHeight: 110 }]}>
                    ...
                    {prioridades.map((p) => (
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          reportePrioridad === p && { backgroundColor: PRIORIDAD_COLORES[p], borderColor: PRIORIDAD_COLORES[p] }
                        ]}
                  ...
                  <TouchableOpacity
                    style={[styles.saveButton, enviandoReporte && { opacity: 0.7 }]}
                    onPress={enviarReporte}
                    disabled={enviandoReporte}
                  >
                    <Text style={styles.saveButtonText}>{enviandoReporte ? "Enviando..." : "Enviar reporte"}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.cancelButton} onPress={() => setModalReporteVisible(false)}>
                    <Text style={styles.cancelButtonText}>Cancelar</Text>
                  </TouchableOpacity>
      ...
      <Modal visible={alertVisible} transparent animationType="fade">
        <View style={styles.alertOverlay}>
          <View style={styles.alertContainer}>
            ...
            <Text style={styles.alertTitulo}>{alertTitulo}</Text>
            <Text style={styles.alertMensaje}>{alertMensaje}</Text>
            <TouchableOpacity style={styles.alertBoton} onPress={() => setAlertVisible(false)}>
...
const styles = StyleSheet.create({
  ...
  avatar: {
    ...
    marginBottom: 14, shadowColor: '#FF6B00',
  ...
  editButton: {
    ...
    backgroundColor: '#FFF3E6', paddingHorizontal: 12, paddingVertical: 6,
    borderRadius: 20, borderWidth: 1, borderColor: '#FF6B00',
  },
  ...
  infoIconContainer: {
    ...
    backgroundColor: '#FFF3E6', justifyContent: 'center', alignItems: 'center', marginRight: 12,
  },
  infoContent: { flex: 1 },
  ...
  actionItem: {
    ...
    backgroundColor: '#F8F9FA', paddingVertical: 14, paddingHorizontal: 16,
    borderRadius: 14, marginBottom: 10, borderWidth: 1, borderColor: '#EEEEEE',
  },
  actionLeft: { flexDirection: 'row', alignItems: 'center' },
  actionIconContainer: { width: 42, height: 42, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginRight: 14 },
  actionText: { fontSize: 15, color: '#333333', fontWeight: '500' },
  actionSubtext: { fontSize: 12, color: '#999999', marginTop: 2 },

  logoutButton: {
    backgroundColor: '#E53935', flexDirection: 'row', justifyContent: 'center',
    alignItems: 'center', paddingVertical: 16, borderRadius: 14, marginTop: 14,
  ...
  prioridadContainer: { flexDirection: 'row', gap: 8, marginTop: 4, flexWrap: 'wrap' },
  prioridadChip: {
    paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20,
    borderWidth: 1, borderColor: '#EEEEEE', backgroundColor: '#F8F9FA',
  },
  prioridadChipText: { fontSize: 13, color: '#666666', fontWeight: '500' },
  prioridadChipTextActivo: { color: '#FFFFFF', fontWeight: '700' },
  ...
  exitoContainer: { alignItems: 'center', paddingVertical: 32 },
  exitoIconContainer: {
    width: 90, height: 90, borderRadius: 45,
    backgroundColor: '#F0FFF4', justifyContent: 'center', alignItems: 'center', marginBottom: 16,
  },
  exitoTitle: { fontSize: 22, fontWeight: '700', color: '#000000', marginBottom: 8 },
  exitoSubtitle: { fontSize: 14, color: '#888888', textAlign: 'center' },
